...
public class Terrain {
	...
	public Terrain(int gridX, int gridZ, Loader loader, TerrainTexturePack texturePack, TerrainTexture blendMap, String heightMap) {
		...
		this.x = gridX * SIZE;
		this.z = gridZ * SIZE;
		this.model = generateTerrainUsingHeightMap(loader, heightMap);
	}
	...
	private RawModel generateTerrainUsingHeightMap(Loader loader, String heightMap) {
		...
		for (int i = 0; i < VERTEX_COUNT; i++) {
			for (int j = 0; j < VERTEX_COUNT; j++) {
				vertices[vertexPointer * 3] = (float) j / ((float) VERTEX_COUNT - 1) * SIZE;
				float height = getHeightUsingHeightMap(j, i, image);
				heights[j][i] = height;
				vertices[vertexPointer * 3 + 1] = height;
				vertices[vertexPointer * 3 + 2] = (float) i / ((float) VERTEX_COUNT - 1) * SIZE;
	...
	private Vector3f calculateNormal(int x, int z, HeightsGenerator generator) {
		...
		Vector3f normal = new Vector3f((heightL - heightR) / 2, 2f, (heightD - heightU) / 2);
		normal.normalise();
		return normal;
	}
	...
	private float getHeightUsingHeightMap(int x, int z, BufferedImage image) {
